...
public class AddItemActivity extends AppCompatActivity implements
    ...
    private void saveItem() {
        ...
        if (mCurrentItemUri == null &&
                ...
                TextUtils.isEmpty(quantityString) && mCategory == ItemEntry.ITEM_CATEGORY_OTHER) {

            // Exit activity
            finish();

            // No fields were modified, don't create a new item.
            return;
        }
        ...
        if (mCurrentItemUri == null) {
            ...
            if (newUri == null) {
                ...
                Toast.makeText(this, getString(R.string.addItem_insert_item_failed),
                        Toast.LENGTH_LONG).show();
            } else {
                // Otherwise, the insertion was successful and we can display a toast.
                Toast.makeText(this, getString(R.string.addItem_insert_item_successful),
                        Toast.LENGTH_LONG).show();
            }
        } else {
            // Otherwise this is an EXISTING item, so update the item.
            int rowsAffected = getContentResolver().update(mCurrentItemUri, values, null, null);

            // Show a toast message depending on whether or not the update was successful.
            if (rowsAffected == 0) {
                // If no rows were affected, then there was an error with the update.
                Toast.makeText(this, getString(R.string.addItem_update_item_failed),
                        Toast.LENGTH_LONG).show();
            } else {
                // Otherwise, the update was successful and we can display a toast.
                Toast.makeText(this, getString(R.string.addItem_update_item_successful),
                        Toast.LENGTH_LONG).show();
            }
        }
        finish();
    ...
    public void onBackPressed() {
        ...
        // If there are unsaved changes, a dialog to warn the user will pop up.
        DialogInterface.OnClickListener discardButtonClickListener =
                new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            ...
            // into one of the dropdown options (0 is other, 1 is food, 2 is drink).
            // Then call setSelection() so that option is displayed on screen as the current selection.
            switch (category) {
                case ItemEntry.ITEM_CATEGORY_FOOD:
                    mCategorySpinner.setSelection(1);
                    break;
                case ItemEntry.ITEM_CATEGORY_DRINK:
